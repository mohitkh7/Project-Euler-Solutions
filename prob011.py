...
def diagonal_down_product(row, col):
    if ((row + 3) < ARR_SIZE) and ((col + 3) < ARR_SIZE):
        return arr[row][col] * arr[row + 1][col + 1] * arr[row + 2][col + 2] * arr[row + 3][col + 3]
    return 0


def diagonal_up_product(row, col):
    if row - 3 >= 0 and col + 3 < ARR_SIZE:
        return arr[row][col] * arr[row - 1][col + 1] * arr[row - 2][col + 2] * arr[row - 3][col + 3]
...
for row in range(20):
    for col in range(20):
        # Up/Down product
        ver = vertical_product(row, col)
        # Left/Right Product
        hor = horizontal_product(row, col)
        d_down = diagonal_down_product(row, col)
        d_up = diagonal_up_product(row, col)
        current_max = max(ver, hor, d_down, d_up)
